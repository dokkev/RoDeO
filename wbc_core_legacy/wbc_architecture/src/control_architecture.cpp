...
bool ControlArchitecture::SolverUpdate() {
  ...
  const int n_active = robot_->NumActiveDof();
  auto t2 = timing ? Clock::time_point{} : Clock::time_point{};

  if (solve_pipeline_mode_ == SolvePipelineMode::kIkThenId) {
    // --- IK/reference solve ---
    ...
    t2 = timing ? Clock::now() : Clock::time_point{};

    // --- Inverse dynamics solve (QP setup + ProxQP + torque recovery) ---
    if (!solver_->SolveInverseDynamics(formulation_, buffers_.qddot_posture_ref,
                                       cmd_.tau)) {
      return false;
    }
  } else {
    // Single ID-QP path (IHWBC-style): no external qddot reference input.
    cmd_.q = robot_->GetJointPos();
    cmd_.qdot = robot_->GetQdotRef().tail(n_active);
    t2 = t1;
    if (!solver_->SolveInverseDynamicsSingleQP(formulation_, cmd_.tau)) {
      return false;
    }
  }
