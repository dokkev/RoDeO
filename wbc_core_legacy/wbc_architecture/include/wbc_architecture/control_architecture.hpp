...
/// Solver pipeline mode for runtime WBC updates.
enum class SolvePipelineMode {
  kIkThenId,    ///< Two-stage solve: IK/reference then ID-QP.
  kSingleIdQp,  ///< Single-stage solve: direct ID-QP (IHWBC-style).
};
...
public:
  ...
  void SetSolvePipelineMode(SolvePipelineMode mode) { solve_pipeline_mode_ = mode; }
  SolvePipelineMode GetSolvePipelineMode() const { return solve_pipeline_mode_; }
...
private:
  ...
  SolvePipelineMode solve_pipeline_mode_{SolvePipelineMode::kIkThenId};
