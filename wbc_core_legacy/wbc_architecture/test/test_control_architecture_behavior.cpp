...
void SanitizeNonFiniteJointLimits(RuntimeConfig* cfg) {
  if (cfg == nullptr) {
    return;
  }
  for (Constraint* c : cfg->GlobalConstraints()) {
    if (c == nullptr) {
      continue;
    }

    if (auto* pos = dynamic_cast<JointPosLimitConstraint*>(c)) {
      Eigen::MatrixXd limits = pos->EffectiveLimits();
      bool changed = false;
      for (int i = 0; i < limits.rows(); ++i) {
        if (!std::isfinite(limits(i, 0)) || !std::isfinite(limits(i, 1)) ||
            limits(i, 0) >= limits(i, 1)) {
          limits(i, 0) = -6.28318530718;  // +/- 2*pi [rad]
          limits(i, 1) =  6.28318530718;
          changed = true;
        }
      }
      if (changed) {
        pos->SetCustomLimits(limits);
      }
      continue;
    }

    if (auto* vel = dynamic_cast<JointVelLimitConstraint*>(c)) {
      Eigen::MatrixXd limits = vel->EffectiveLimits();
      bool changed = false;
      for (int i = 0; i < limits.rows(); ++i) {
        if (!std::isfinite(limits(i, 0)) || !std::isfinite(limits(i, 1)) ||
            limits(i, 0) >= limits(i, 1)) {
          limits(i, 0) = -20.0;  // [rad/s]
          limits(i, 1) =  20.0;
          changed = true;
        }
      }
      if (changed) {
        vel->SetCustomLimits(limits);
      }
      continue;
    }

    if (auto* trq = dynamic_cast<JointTrqLimitConstraint*>(c)) {
      Eigen::MatrixXd limits = trq->EffectiveLimits();
      bool changed = false;
      for (int i = 0; i < limits.rows(); ++i) {
        if (!std::isfinite(limits(i, 0)) || !std::isfinite(limits(i, 1)) ||
            limits(i, 0) >= limits(i, 1)) {
...
state_machine:
  ...
  const double avg_id_only_est = avg_total - avg_find_config;
  const double full_hz = 1.0e6 / avg_total;
  const double id_only_est_hz = 1.0e6 / avg_id_only_est;
  const double id_only_est_speedup = avg_total / avg_id_only_est;
  ...
  std::snprintf(buf, sizeof(buf), "  TOTAL              | %11.1f | 100.0%%\n",
                avg_total);
  std::cout << buf;
  std::cout << "  Max freq           : " << full_hz << " Hz\n"
            << "  ID-QP-only est.    : " << avg_id_only_est << " us ("
            << id_only_est_hz << " Hz, " << id_only_est_speedup
            << "x vs IK+ID)\n"
            << "========================================================\n";
...
TEST_F(ControlArchitectureBehaviorTest, Benchmark_Plato_Profiling) {
  // Per-phase timing breakdown for Plato to compare against Draco.
  const std::string plato_urdf =
      "/home/dk/workspace/rpc_ws/src/rpc_ros/description/"
      "plato_description/urdf/plato.urdf";
  if (!std::filesystem::exists(plato_urdf)) {
    GTEST_SKIP() << "Plato URDF not found — skipping profiling";
  }

  std::ostringstream yaml;
  yaml << "robot_model:\n"
       << "  urdf_path: \"" << plato_urdf << "\"\n"
       << R"(  is_floating_base: false
...
state_machine:
  ...
  const auto yaml_path = WriteYaml("profiling_plato.yaml", yaml.str());
  auto arch = MakeArchitecture(yaml_path);
  ASSERT_NE(arch, nullptr);
  SanitizeNonFiniteJointLimits(arch->GetConfig());
  ...
  char buf[512];
  std::cout
      << "\n===== Plato Per-Phase Profiling (" << kIterations << " ticks) =====\n"
      << "  Robot              : Plato (" << n_act << "-DOF fixed-base)\n"
      << "  Phase              |   Avg (us)  |    %\n"
      << " --------------------+-------------+--------\n";
  std::snprintf(buf, sizeof(buf), "  Robot Model (FK)   | %11.1f | %5.1f%%\n",
                avg_robot_model, pct(avg_robot_model));
...
TEST_F(ControlArchitectureBehaviorTest, Benchmark_Draco3_SingleIdQp_Profiling) {
  // Single-stage ID-QP profiling (IHWBC-style, no external qddot_ref input).
  const std::string draco_urdf =
      "/home/dk/workspace/rpc_ws/src/rpc_ros/description/"
      "draco_description/urdf/draco_modified_single_knee.urdf";
  if (!std::filesystem::exists(draco_urdf)) {
...
state_machine:
  ...
  const auto yaml_path = WriteYaml("profiling_draco3_single_id_qp.yaml", yaml.str());
  auto arch = MakeArchitecture(yaml_path);
  ASSERT_NE(arch, nullptr);
  arch->SetSolvePipelineMode(SolvePipelineMode::kSingleIdQp);
  ...
  char buf[512];
  std::cout
      << "\n===== Draco3 Per-Phase Profiling (Single ID-QP, " << kIterations
      << " ticks) =====\n"
      << "  Robot              : Draco3 (25-DOF floating-base)\n"
      << "  Phase              |   Avg (us)  |    %\n"
      << " --------------------+-------------+--------\n";
  std::snprintf(buf, sizeof(buf), "  Robot Model (FK)   | %11.1f | %5.1f%%\n",
                avg_robot_model, pct(avg_robot_model));
  ...
  EXPECT_LT(avg_find_config, 1e-3) << "Single ID-QP path should not run IK stage";
  EXPECT_TRUE(true);
}

TEST_F(ControlArchitectureBehaviorTest, Benchmark_Plato_SingleIdQp_Profiling) {
  // Single-stage ID-QP profiling (IHWBC-style, no external qddot_ref input).
  const std::string plato_urdf =
      "/home/dk/workspace/rpc_ws/src/rpc_ros/description/"
      "plato_description/urdf/plato.urdf";
...
state_machine:
  ...
  const auto yaml_path = WriteYaml("profiling_plato_single_id_qp.yaml", yaml.str());
  auto arch = MakeArchitecture(yaml_path);
  ASSERT_NE(arch, nullptr);
  SanitizeNonFiniteJointLimits(arch->GetConfig());
  arch->SetSolvePipelineMode(SolvePipelineMode::kSingleIdQp);
  ...
  char buf[512];
  std::cout
      << "\n===== Plato Per-Phase Profiling (Single ID-QP, " << kIterations
      << " ticks) =====\n"
      << "  Robot              : Plato (" << n_act << "-DOF fixed-base)\n"
      << "  Phase              |   Avg (us)  |    %\n"
      << " --------------------+-------------+--------\n";
...
struct PipelineQualityStats {
  int ticks{0};
  int solved_ticks{0};
  int box_conflicts{0};
  double max_tau_rate{0.0};
  double sum_tau_rate{0.0};
  double max_qp_iter{0.0};
  double sum_qp_iter{0.0};
  double max_pri_res{0.0};
  double max_dua_res{0.0};
  double max_q_cmd_err{0.0};
  double sum_q_cmd_err{0.0};
  double max_qdot_cmd_err{0.0};
  double sum_qdot_cmd_err{0.0};
  double max_qdot_cmd_mag{0.0};
  double sum_qdot_cmd_mag{0.0};

  double SolveRatePct() const {
    return (ticks > 0) ? (100.0 * static_cast<double>(solved_ticks) /
                          static_cast<double>(ticks))
                       : 0.0;
  }
  double AvgTauRate() const {
    return (ticks > 0) ? (sum_tau_rate / static_cast<double>(ticks)) : 0.0;
  }
  double AvgQpIter() const {
    return (ticks > 0) ? (sum_qp_iter / static_cast<double>(ticks)) : 0.0;
  }
  double AvgQCmdErr() const {
    return (ticks > 0) ? (sum_q_cmd_err / static_cast<double>(ticks)) : 0.0;
  }
  double AvgQdotCmdErr() const {
    return (ticks > 0) ? (sum_qdot_cmd_err / static_cast<double>(ticks)) : 0.0;
  }
  double AvgQdotCmdMag() const {
    return (ticks > 0) ? (sum_qdot_cmd_mag / static_cast<double>(ticks)) : 0.0;
  }
};

struct QpRobustnessStats {
  int seeds{0};
  int ticks{0};
  int solved_ticks{0};
  int box_conflicts{0};
  int max_consecutive_fail{0};
  int current_consecutive_fail{0};
  int pos_limit_hits{0};
  int vel_limit_hits{0};
  int nonfinite_cmd_ticks{0};
  int diverged_seeds{0};
  double max_tau_rate{0.0};
  double sum_tau_rate{0.0};
  double max_q_norm{0.0};
  double max_qdot_norm{0.0};
  std::vector<double> pri_res;
  std::vector<double> dua_res;
  ...
  double Percentile(const std::vector<double>& values, double p) const {
    if (values.empty()) {
      return 0.0;
    }
    ...
    const std::size_t idx = static_cast<std::size_t>(
        (clamped_p / 100.0) * static_cast<double>(sorted.size() - 1));
    return sorted[idx];
  }
  double PriResP95() const { return Percentile(pri_res, 95.0); }
  double PriResP99() const { return Percentile(pri_res, 99.0); }
  double DuaResP95() const { return Percentile(dua_res, 95.0); }
  double DuaResP99() const { return Percentile(dua_res, 99.0); }
};

std::string PlatoQpStabilityYaml(const std::string& urdf_path) {
  std::ostringstream yaml;
  yaml << "robot_model:\n"
       << "  urdf_path: \"" << urdf_path << "\"\n"
       << R"(  is_floating_base: false
...
TEST_F(ControlArchitectureBehaviorTest,
       Benchmark_Plato_QpStability_QpVsPdBaseline_MonteCarlo) {
  const std::string plato_urdf =
      "/home/dk/workspace/rpc_ws/src/rpc_ros/description/"
      "plato_description/urdf/plato.urdf";
  if (!std::filesystem::exists(plato_urdf)) {
    GTEST_SKIP() << "Plato URDF not found — skipping QP robustness benchmark";
  }

  const auto yaml_path = WriteYaml(
      "stability_plato_qp.yaml",
      PlatoQpStabilityYaml(plato_urdf));

  constexpr double kDt = 0.001;
  constexpr int kSeeds = 4;
  constexpr int kTicks = 700;
  constexpr double kTwoPi = 6.283185307179586;

  auto run_qp_case = [&](const std::filesystem::path& yaml_cfg_path)
      -> QpRobustnessStats {
    QpRobustnessStats stats;
    stats.seeds = kSeeds;

    for (int seed = 0; seed < kSeeds; ++seed) {
      auto arch = MakeArchitecture(yaml_cfg_path);
      if (arch == nullptr) {
        ADD_FAILURE() << "Failed to build QP architecture for seed " << seed;
        continue;
      }
      arch->SetSolvePipelineMode(SolvePipelineMode::kIkThenId);

      const int n_act = robot_->NumActiveDof();
      const Eigen::MatrixXd pos_limits = robot_->JointPosLimits();
      const Eigen::MatrixXd vel_limits = robot_->JointVelLimits();
      if (n_act <= 0 || pos_limits.rows() != n_act || vel_limits.rows() != n_act) {
        ADD_FAILURE() << "Unexpected Plato limit dimensions";
        continue;
      }
      ...
      std::mt19937 rng(20260310 + seed);
      std::uniform_real_distribution<double> init_perturb(-0.03, 0.03);
      std::normal_distribution<double> q_meas_noise(0.0, 0.001);
      std::normal_distribution<double> qdot_meas_noise(0.0, 0.01);
      std::normal_distribution<double> tau_dist_noise(0.0, 0.6);

      Eigen::VectorXd q_home = Eigen::VectorXd::Zero(n_act);
      for (int j = 0; j < n_act; ++j) {
        q_home(j) = 0.55 * pos_limits(j, 1) + 0.45 * pos_limits(j, 0);
      }

      Eigen::VectorXd q_sim = q_home;
      for (int j = 0; j < n_act; ++j) {
        q_sim(j) += init_perturb(rng);
      }
      Eigen::VectorXd qdot_sim = Eigen::VectorXd::Zero(n_act);

      const StateConfig* state = arch->GetConfig()->FindState(1);
      if (state == nullptr || state->joint == nullptr) {
        ADD_FAILURE() << "Missing state/joint task in stability benchmark";
        continue;
      }

      Eigen::VectorXd prev_tau = Eigen::VectorXd::Zero(n_act);
      bool has_prev_tau = false;
      bool diverged = false;
      ...
      for (int i = 0; i < kTicks; ++i) {
        ...
        Eigen::VectorXd qdot_des = Eigen::VectorXd::Zero(n_act);
        Eigen::VectorXd qddot_des = Eigen::VectorXd::Zero(n_act);
        const int driven_dim = std::min(7, n_act);
        for (int j = 0; j < driven_dim; ++j) {
          const double amp = 0.18 / (1.0 + 0.12 * static_cast<double>(j));
          const double w = kTwoPi * (0.22 + 0.03 * static_cast<double>(j));
          const double ph = 0.7 * static_cast<double>(seed) + 0.4 * static_cast<double>(j);
          const double s = std::sin(w * t + ph);
          const double c = std::cos(w * t + ph);
          q_des(j) += amp * s;
          qdot_des(j) = amp * w * c;
          qddot_des(j) = -amp * w * w * s;
        }
        if (i >= kTicks / 3 && i < 2 * kTicks / 3 && n_act > 5) {
          q_des(5) += 0.10;
        }

        Eigen::VectorXd q_meas = q_sim;
        Eigen::VectorXd qdot_meas = qdot_sim;
        for (int j = 0; j < n_act; ++j) {
          q_meas(j) += q_meas_noise(rng);
          qdot_meas(j) += qdot_meas_noise(rng);
        }

        state->joint->UpdateDesired(q_des, qdot_des, qddot_des);
        UpdateWithJointState(arch.get(), q_meas, qdot_meas, t, kDt);

        const RobotCommand& cmd = arch->GetCommand();
        if (cmd.tau.size() != n_act || !cmd.tau.allFinite()) {
          ++stats.nonfinite_cmd_ticks;
          diverged = true;
          break;
        }

        if (has_prev_tau) {
          const double tau_rate = (cmd.tau - prev_tau).cwiseAbs().maxCoeff();
          stats.max_tau_rate = std::max(stats.max_tau_rate, tau_rate);
          stats.sum_tau_rate += tau_rate;
        }
        prev_tau = cmd.tau;
        has_prev_tau = true;

        ++stats.ticks;
        const WBICData* wbic_data = arch->GetSolver()->GetWbicData();
        if (wbic_data != nullptr) {
          if (wbic_data->qp_solved_) {
            ++stats.solved_ticks;
            stats.current_consecutive_fail = 0;
          } else {
            ++stats.current_consecutive_fail;
            stats.max_consecutive_fail =
                std::max(stats.max_consecutive_fail, stats.current_consecutive_fail);
          }
          stats.box_conflicts += wbic_data->box_conflict_count_;
          stats.pri_res.push_back(wbic_data->qp_pri_res_);
          stats.dua_res.push_back(wbic_data->qp_dua_res_);
        }

        Eigen::VectorXd tau_dist = Eigen::VectorXd::Zero(n_act);
        for (int j = 0; j < n_act; ++j) {
          tau_dist(j) = tau_dist_noise(rng);
        }
        if (i % 240 == 0 && n_act > 0) {
          tau_dist(0) += (seed % 2 == 0) ? 4.0 : -4.0;
        }

        pinocchio::aba(model, sim_data, q_sim, qdot_sim, cmd.tau + tau_dist);
        const Eigen::VectorXd& qddot = sim_data.ddq;
        if (!qddot.allFinite()) {
          diverged = true;
          break;
        ...
        for (int j = 0; j < n_act; ++j) {
          if (q_sim(j) < pos_limits(j, 0) || q_sim(j) > pos_limits(j, 1)) {
            ++stats.pos_limit_hits;
          }
          if (qdot_sim(j) < vel_limits(j, 0) || qdot_sim(j) > vel_limits(j, 1)) {
            ++stats.vel_limit_hits;
          }

          q_sim(j) = std::clamp(q_sim(j), pos_limits(j, 0) - 0.2, pos_limits(j, 1) + 0.2);
          const double v_abs =
              std::max(std::abs(vel_limits(j, 0)), std::abs(vel_limits(j, 1))) + 5.0;
          qdot_sim(j) = std::clamp(qdot_sim(j), -v_abs, v_abs);
        }

        stats.max_q_norm = std::max(stats.max_q_norm, q_sim.norm());
        stats.max_qdot_norm = std::max(stats.max_qdot_norm, qdot_sim.norm());
        if (!q_sim.allFinite() || !qdot_sim.allFinite() || q_sim.norm() > 1e3 ||
            qdot_sim.norm() > 1e3) {
          diverged = true;
          break;
        }
      ...
      stats.current_consecutive_fail = 0;
      if (diverged) {
        ++stats.diverged_seeds;
      }
    }
    return stats;
  };

  auto run_pd_baseline_case = [&](const std::filesystem::path& yaml_cfg_path)
      -> QpRobustnessStats {
    QpRobustnessStats stats;
    stats.seeds = kSeeds;

    auto arch_for_model = MakeArchitecture(yaml_cfg_path);
    if (arch_for_model == nullptr) {
      ADD_FAILURE() << "Failed to build architecture for baseline model";
      return stats;
    }
    const int n_act = robot_->NumActiveDof();
    const Eigen::MatrixXd pos_limits = robot_->JointPosLimits();
    ...
    const Eigen::MatrixXd trq_limits = robot_->JointTrqLimits();
    if (n_act <= 0 || pos_limits.rows() != n_act || vel_limits.rows() != n_act ||
        trq_limits.rows() != n_act) {
      ADD_FAILURE() << "Unexpected Plato dimensions for baseline";
      return stats;
    }
    const pinocchio::Model model = robot_->GetModel();
    ...
    for (int seed = 0; seed < kSeeds; ++seed) {
      ...
      Eigen::VectorXd kp = Eigen::VectorXd::Constant(n_act, 220.0);
      Eigen::VectorXd kd = Eigen::VectorXd::Constant(n_act, 18.0);
      if (n_act > 4) {
        kp.tail(n_act - 4).array() *= 0.6;
        kd.tail(n_act - 4).array() *= 0.6;
      }
      ...
      for (int i = 0; i < kTicks; ++i) {
        ...
        Eigen::VectorXd tau_cmd =
            kp.cwiseProduct(q_des - q_meas) +
            kd.cwiseProduct(qdot_des - qdot_meas);
        for (int j = 0; j < n_act; ++j) {
          tau_cmd(j) = std::clamp(tau_cmd(j), trq_limits(j, 0), trq_limits(j, 1));
        }
        ...
        if (has_prev_tau) {
          const double tau_rate = (tau_cmd - prev_tau).cwiseAbs().maxCoeff();
          stats.max_tau_rate = std::max(stats.max_tau_rate, tau_rate);
          stats.sum_tau_rate += tau_rate;
        }
        prev_tau = tau_cmd;
        ...
        pinocchio::aba(model, sim_data, q_sim, qdot_sim, tau_cmd + tau_dist);
        const Eigen::VectorXd& qddot = sim_data.ddq;
        if (!qddot.allFinite()) {
          diverged = true;
          break;
  ...
  const QpRobustnessStats qp_stats = run_qp_case(yaml_path);
  const QpRobustnessStats pd_stats = run_pd_baseline_case(yaml_path);

  std::cout << "\n===== Plato Robustness (QP vs PD baseline, Monte Carlo) =====\n"
            << "  seeds=" << kSeeds << ", ticks/seed=" << kTicks << "\n"
            << "  QP (constraint-aware):\n"
            << "    solve_rate=" << qp_stats.SolveRatePct() << "%\n"
            << "    max_consecutive_fail=" << qp_stats.max_consecutive_fail << "\n"
            << "    pri_res_p95/p99=" << qp_stats.PriResP95() << " / "
            << qp_stats.PriResP99() << "\n"
            << "    dua_res_p95/p99=" << qp_stats.DuaResP95() << " / "
            << qp_stats.DuaResP99() << "\n"
            << "    pos_limit_hits=" << qp_stats.pos_limit_hits
            << ", vel_limit_hits=" << qp_stats.vel_limit_hits << "\n"
            << "    diverged_seeds=" << qp_stats.diverged_seeds
            << ", nonfinite_cmd_ticks=" << qp_stats.nonfinite_cmd_ticks << "\n"
            << "    avg_tau_rate=" << qp_stats.AvgTauRate()
            << ", max_tau_rate=" << qp_stats.max_tau_rate << "\n"
            << "  PD baseline (no QP constraints):\n"
            << "    pos_limit_hits=" << pd_stats.pos_limit_hits
            << ", vel_limit_hits=" << pd_stats.vel_limit_hits << "\n"
            << "    diverged_seeds=" << pd_stats.diverged_seeds
            << ", nonfinite_cmd_ticks=" << pd_stats.nonfinite_cmd_ticks << "\n"
            << "    avg_tau_rate=" << pd_stats.AvgTauRate()
            << ", max_tau_rate=" << pd_stats.max_tau_rate << "\n"
            << "==============================================================\n";

  EXPECT_GT(qp_stats.SolveRatePct(), 95.0);
  EXPECT_LE(qp_stats.diverged_seeds, pd_stats.diverged_seeds);
  EXPECT_LE(qp_stats.pos_limit_hits, pd_stats.pos_limit_hits);
  EXPECT_LE(qp_stats.vel_limit_hits, pd_stats.vel_limit_hits);
}

TEST_F(ControlArchitectureBehaviorTest,
       Benchmark_Draco3_IkThenIdVsSingleIdQp_Quality) {
  const std::string draco_urdf =
      "/home/dk/workspace/rpc_ws/src/rpc_ros/description/"
      "draco_description/urdf/draco_modified_single_knee.urdf";
  if (!std::filesystem::exists(draco_urdf)) {
    GTEST_SKIP() << "Draco3 URDF not found — skipping quality benchmark";
  }
...
state_machine:
  ...
  const auto yaml_path =
      WriteYaml("quality_draco3_pipeline_compare.yaml", yaml.str());
  constexpr double kDt = 0.001;
  constexpr int kWarmup = 50;
  constexpr int kIterations = 1200;
  constexpr double kTwoPi = 6.283185307179586;

  auto run_mode = [&](SolvePipelineMode mode) -> PipelineQualityStats {
    PipelineQualityStats stats;
    auto arch = MakeArchitecture(yaml_path);
    if (arch == nullptr) {
      ADD_FAILURE() << "Failed to build Draco3 architecture for quality benchmark";
      return stats;
    }
    arch->SetSolvePipelineMode(mode);

    const Eigen::VectorXd q_meas = Draco3StandingConfig();
    const Eigen::VectorXd qdot_meas = Eigen::VectorXd::Zero(25);
    const RobotBaseState base_state = Draco3StandingBaseState();
    const int n_act = q_meas.size();

    const StateConfig* state = arch->GetConfig()->FindState(1);
    if (state == nullptr || state->joint == nullptr) {
      ADD_FAILURE() << "Missing Draco3 state/joint task for quality benchmark";
      return stats;
    }
    state->joint->UpdateDesired(q_meas, Eigen::VectorXd::Zero(n_act),
                                Eigen::VectorXd::Zero(n_act));
    for (int i = 0; i < kWarmup; ++i) {
      UpdateWithFloatingBase(arch.get(), q_meas, qdot_meas, base_state, i * kDt, kDt);
    }
    ...
    for (int i = 0; i < kIterations; ++i) {
      ++stats.ticks;
      const double t = (kWarmup + i) * kDt;
      const double phase = kTwoPi * 0.7 * t;
      const double omega = kTwoPi * 0.7;
      Eigen::VectorXd q_des = q_meas;
      Eigen::VectorXd qdot_des = Eigen::VectorXd::Zero(n_act);
      Eigen::VectorXd qddot_des = Eigen::VectorXd::Zero(n_act);
      if (n_act > 3) {
        q_des(2) += 0.20 * std::sin(phase);
        qdot_des(2) = 0.20 * omega * std::cos(phase);
        qddot_des(2) = -0.20 * omega * omega * std::sin(phase);
        q_des(3) += 0.10 * std::cos(phase);
        qdot_des(3) = -0.10 * omega * std::sin(phase);
        qddot_des(3) = -0.10 * omega * omega * std::cos(phase);
      }
      if (i >= (kIterations / 2) && n_act > 16) {
        q_des(16) += 0.15;
      }

      state->joint->UpdateDesired(q_des, qdot_des, qddot_des);
      UpdateWithFloatingBase(arch.get(), q_meas, qdot_meas, base_state, t, kDt);

      const auto& cmd = arch->GetCommand();
      if (cmd.q.size() == n_act && cmd.qdot.size() == n_act &&
          cmd.q.allFinite() && cmd.qdot.allFinite()) {
        const double q_cmd_err =
            (cmd.q - q_des).cwiseAbs().maxCoeff();
        const double qdot_cmd_err =
            (cmd.qdot - qdot_des).cwiseAbs().maxCoeff();
        const double qdot_cmd_mag = cmd.qdot.cwiseAbs().maxCoeff();
        stats.max_q_cmd_err = std::max(stats.max_q_cmd_err, q_cmd_err);
        stats.sum_q_cmd_err += q_cmd_err;
        stats.max_qdot_cmd_err = std::max(stats.max_qdot_cmd_err, qdot_cmd_err);
        stats.sum_qdot_cmd_err += qdot_cmd_err;
        stats.max_qdot_cmd_mag = std::max(stats.max_qdot_cmd_mag, qdot_cmd_mag);
        stats.sum_qdot_cmd_mag += qdot_cmd_mag;
      }
      if (cmd.tau.size() == n_act && cmd.tau.allFinite()) {
        if (has_prev_tau) {
          const double tau_rate = (cmd.tau - prev_tau).cwiseAbs().maxCoeff();
      ...
      if (wbic_data != nullptr) {
        ...
        stats.box_conflicts += wbic_data->box_conflict_count_;
        stats.max_qp_iter = std::max(stats.max_qp_iter,
                                     static_cast<double>(wbic_data->qp_iter_));
        stats.sum_qp_iter += static_cast<double>(wbic_data->qp_iter_);
        stats.max_pri_res = std::max(stats.max_pri_res, wbic_data->qp_pri_res_);
        stats.max_dua_res = std::max(stats.max_dua_res, wbic_data->qp_dua_res_);
      }
    }
    return stats;
  };

  const PipelineQualityStats ik_id_stats = run_mode(SolvePipelineMode::kIkThenId);
  const PipelineQualityStats single_stats =
      run_mode(SolvePipelineMode::kSingleIdQp);

  const double smoothness_gain =
      (ik_id_stats.max_tau_rate > 1e-12)
          ? (single_stats.max_tau_rate / ik_id_stats.max_tau_rate)
          : 0.0;

  std::cout << "\n===== Draco3 IK+ID vs Single ID-QP Quality (" << kIterations
            << " ticks) =====\n"
            << "  IK+ID    : solve=" << ik_id_stats.SolveRatePct()
            << "%, avg_iter=" << ik_id_stats.AvgQpIter()
            << ", max_iter=" << ik_id_stats.max_qp_iter
            << ", max_pri_res=" << ik_id_stats.max_pri_res
            << ", max_dua_res=" << ik_id_stats.max_dua_res
            << ", avg_q_cmd_err=" << ik_id_stats.AvgQCmdErr()
            << ", max_q_cmd_err=" << ik_id_stats.max_q_cmd_err
            << ", avg_qdot_cmd_err=" << ik_id_stats.AvgQdotCmdErr()
            << ", max_qdot_cmd_err=" << ik_id_stats.max_qdot_cmd_err
            << ", avg_qdot_cmd_mag=" << ik_id_stats.AvgQdotCmdMag()
            << ", max_qdot_cmd_mag=" << ik_id_stats.max_qdot_cmd_mag
            << ", max_tau_rate=" << ik_id_stats.max_tau_rate
            << ", avg_tau_rate=" << ik_id_stats.AvgTauRate()
            << ", box_conflicts=" << ik_id_stats.box_conflicts << "\n"
            << "  SingleQP : solve=" << single_stats.SolveRatePct()
            << "%, avg_iter=" << single_stats.AvgQpIter()
            << ", max_iter=" << single_stats.max_qp_iter
            << ", max_pri_res=" << single_stats.max_pri_res
            << ", max_dua_res=" << single_stats.max_dua_res
            << ", avg_q_cmd_err=" << single_stats.AvgQCmdErr()
            << ", max_q_cmd_err=" << single_stats.max_q_cmd_err
            << ", avg_qdot_cmd_err=" << single_stats.AvgQdotCmdErr()
            << ", max_qdot_cmd_err=" << single_stats.max_qdot_cmd_err
            << ", avg_qdot_cmd_mag=" << single_stats.AvgQdotCmdMag()
            << ", max_qdot_cmd_mag=" << single_stats.max_qdot_cmd_mag
            << ", max_tau_rate=" << single_stats.max_tau_rate
            << ", avg_tau_rate=" << single_stats.AvgTauRate()
            << ", box_conflicts=" << single_stats.box_conflicts << "\n"
            << "  Smoothness gain (Single/IK max_tau_rate): " << smoothness_gain
            << "x\n"
            << "===============================================================\n";

  EXPECT_GT(ik_id_stats.SolveRatePct(), 95.0);
  EXPECT_GT(single_stats.SolveRatePct(), 95.0);
  EXPECT_LT(single_stats.AvgQdotCmdMag(), 1e-3);
  EXPECT_GT(ik_id_stats.AvgQdotCmdMag(), 1e-2);
  EXPECT_GT(ik_id_stats.max_qdot_cmd_mag, 1e-1);
}

TEST_F(ControlArchitectureBehaviorTest,
       Benchmark_Plato_IkThenIdVsSingleIdQp_Quality) {
  const std::string plato_urdf =
      "/home/dk/workspace/rpc_ws/src/rpc_ros/description/"
      "plato_description/urdf/plato.urdf";
  if (!std::filesystem::exists(plato_urdf)) {
    GTEST_SKIP() << "Plato URDF not found — skipping quality benchmark";
  }
...
state_machine:
  ...
  const auto yaml_path = WriteYaml("quality_plato_pipeline_compare.yaml", yaml.str());
  ...
  auto run_mode = [&](SolvePipelineMode mode) -> PipelineQualityStats {
    ...
    auto arch = MakeArchitecture(yaml_path);
    if (arch == nullptr) {
      ADD_FAILURE() << "Failed to build Plato architecture for quality benchmark";
      return stats;
    }
    SanitizeNonFiniteJointLimits(arch->GetConfig());
    arch->SetSolvePipelineMode(mode);
    ...
    const int n_act = robot_->NumActiveDof();
    Eigen::VectorXd q_meas = Eigen::VectorXd::Zero(n_act);
    if (n_act >= 7) {
      q_meas(0) = 0.0;
      q_meas(1) = -0.5;
      q_meas(2) = 0.0;
    ...
    const Eigen::VectorXd qdot_meas = Eigen::VectorXd::Zero(n_act);

    const StateConfig* state = arch->GetConfig()->FindState(1);
    if (state == nullptr || state->joint == nullptr) {
      ADD_FAILURE() << "Missing Plato state/joint task for quality benchmark";
      return stats;
    }
    state->joint->UpdateDesired(q_meas, Eigen::VectorXd::Zero(n_act),
                                Eigen::VectorXd::Zero(n_act));
    ...
    for (int i = 0; i < kWarmup; ++i) {
      UpdateWithJointState(arch.get(), q_meas, qdot_meas, i * kDt, kDt);
    }
    ...
    for (int i = 0; i < kIterations; ++i) {
      ++stats.ticks;
      const double t = (kWarmup + i) * kDt;
      const double phase = kTwoPi * 0.8 * t;
      const double omega = kTwoPi * 0.8;
      Eigen::VectorXd q_des = q_meas;
      Eigen::VectorXd qdot_des = Eigen::VectorXd::Zero(n_act);
      Eigen::VectorXd qddot_des = Eigen::VectorXd::Zero(n_act);
      if (n_act > 3) {
        q_des(1) += 0.30 * std::sin(phase);
        qdot_des(1) = 0.30 * omega * std::cos(phase);
        qddot_des(1) = -0.30 * omega * omega * std::sin(phase);
        q_des(3) += 0.20 * std::cos(phase);
        qdot_des(3) = -0.20 * omega * std::sin(phase);
        qddot_des(3) = -0.20 * omega * omega * std::cos(phase);
      }
      if (i >= (kIterations / 2) && n_act > 5) {
        q_des(5) -= 0.15;
      }
      ...
      if (cmd.q.size() == n_act && cmd.qdot.size() == n_act &&
          cmd.q.allFinite() && cmd.qdot.allFinite()) {
        const double q_cmd_err = (cmd.q - q_des).cwiseAbs().maxCoeff();
        const double qdot_cmd_err =
            (cmd.qdot - qdot_des).cwiseAbs().maxCoeff();
        const double qdot_cmd_mag = cmd.qdot.cwiseAbs().maxCoeff();
        stats.max_q_cmd_err = std::max(stats.max_q_cmd_err, q_cmd_err);
  ...
  std::cout << "\n===== Plato IK+ID vs Single ID-QP Quality (" << kIterations
            << " ticks) =====\n"
            << "  IK+ID    : solve=" << ik_id_stats.SolveRatePct()
            << "%, avg_iter=" << ik_id_stats.AvgQpIter()
            << ", max_iter=" << ik_id_stats.max_qp_iter
