...
public:
  ...
  /**
   * @brief Single ID-QP solve path without external qddot reference input.
   *
   * @details
   * This matches IHWBC-style usage where acceleration is solved directly in one
   * ID-QP stage. Internally, WBIC uses a zero nominal acceleration reference.
   */
  bool SolveInverseDynamicsSingleQP(const WbcFormulation& formulation,
                                    Eigen::VectorXd& jtrq_cmd);

  /**
   * @brief Structured-output overload of SolveInverseDynamicsSingleQP.
   */
  bool SolveInverseDynamicsSingleQP(const WbcFormulation& formulation,
                                    InverseDynamicsSolution& out_sol);
...
private:
  ...
  Eigen::VectorXd qddot_zero_ref_;
