...
WBIC::WBIC(const std::vector<bool>& act_qdot_list, QPParams* qp_params)
    : WBC(act_qdot_list),
      ...
      qddot_zero_ref_(Eigen::VectorXd::Zero(num_qdot_)),
...
bool WBIC::SolveInverseDynamicsSingleQP(const WbcFormulation& formulation,
                                        Eigen::VectorXd& jtrq_cmd) {
  assert(qddot_zero_ref_.size() == num_qdot_);
  return SolveInverseDynamics(formulation, qddot_zero_ref_, jtrq_cmd);
}

bool WBIC::SolveInverseDynamicsSingleQP(const WbcFormulation& formulation,
                                        InverseDynamicsSolution& out_sol) {
  Eigen::VectorXd jtrq_cmd;
  if (!SolveInverseDynamicsSingleQP(formulation, jtrq_cmd)) {
    return false;
  }
  out_sol.qddot_sol = wbic_data_->qddot_sol_;
  out_sol.rf_sol = wbic_data_->rf_sol_;
