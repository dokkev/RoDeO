...
#include "wbc_core/formulations/inverse-dynamics-formulation-acc-force.hpp"
...
InverseDynamicsFormulationAccForce::InverseDynamicsFormulationAccForce(
    ...
    : InverseDynamicsBase(name, robot, verbose),
      m_data(robot.model()),
...
void InverseDynamicsFormulationAccForce::addTask(TaskLevelPointer tl,
  ...
  while (m_hqpData.size() <= priorityLevel) {
    m_hqpData.push_back(ConstraintLevel());
  }
...
const HQPData& InverseDynamicsFormulationAccForce::computeProblemData(
  ...
  for (auto it_ct = m_contactTransitions.begin();
    ...
    if (m_t <= c->time_end) {
      ...
      it_ct = m_contactTransitions.erase(it_ct);
    }
  }
...
bool InverseDynamicsFormulationAccForce::removeRigidContact(
  ...
  auto motion_priority = getTaskPriority(contactName + "_motion_task");
  bool is_motion_contraint =
      (motion_priority.has_value() && *motion_priority == 0);
...
std::optional<unsigned int> InverseDynamicsFormulationAccForce::getTaskPriority(
    const std::string& name) {
