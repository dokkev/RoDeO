...
bool UsesMeshesPackageAlias(const std::filesystem::path& urdf_path) {
  ...
  while (std::getline(in, line)) {
    if (line.find("package://meshes/") != std::string::npos) return true;
  }
  return false;
}
...
bool UsesRelativeMeshPaths(const std::filesystem::path& urdf_path) {
  ...
  while (std::getline(in, line)) {
    ...
    pos += 10;
    if (line.substr(pos, 10) != "package://") return true;
  }
  return false;
}
...
std::string ResolveUrdfPackageRoot(const std::string& urdf_uri,
  ...
  if (IsPackageUri(urdf_uri)) {
    ...
    const std::filesystem::path share_dir =
        ament_index_cpp::get_package_share_directory(package_name);
    if (uses_mesh_alias) return share_dir.string();
    return share_dir.parent_path().string();
  }

  if (uses_mesh_alias) {
    const std::filesystem::path urdf_dir = urdf_path.parent_path();
    if (!urdf_dir.empty()) return urdf_dir.parent_path().string();
  }

  std::filesystem::path prefix;
  for (const auto& part : urdf_path) {
    prefix /= part;
    if (part == "share") return prefix.string();
  }
