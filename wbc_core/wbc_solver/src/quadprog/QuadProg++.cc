...
void print_matrix(const char *name, const GMatr<double> &A, int n = -1,
                  int m = -1);
...
double solve_quadprog(GMatr<double> &G, GVect<double> &g0,
  ...
  const int n = static_cast<int>(G.ncols());
  const int p = static_cast<int>(CE.ncols());
  const int m = static_cast<int>(CI.ncols());
  if (static_cast<int>(G.nrows()) != n) {
    msg << "The matrix G is not a squared matrix (" << G.nrows() << " x "
        << G.ncols() << ")";
    throw std::logic_error(msg.str());
  }
  if (static_cast<int>(CE.nrows()) != n) {
  ...
  int iq, iter = 0;
  const int max_iter = std::max(50, 20 * (n + p + m));
...
void print_matrix(const char *name, const GMatr<double> &A, int n, int m) {
  std::ostringstream s;
  std::string t;
  if (n == -1)
    n = A.nrows();
