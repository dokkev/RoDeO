# IDHQP Final Architecture Plan

## Goal

Define the **final-form IDHQP** as a **nominal-aware strict-priority inverse-dynamics HQP controller**.
...
* strategy / prediction / policy generation inside IDHQP
...
IDHQP must not rely on a mandatory standalone IK-based redundancy-resolution stage.
...
IDHQP must **not** directly assemble low-level formulation matrices as its main design pattern.
...
class NominalAccelerationProvider {
 public:
  ...
  virtual bool compute(double time,
                       ...
                       const ContactConstraintData& contacts,
                       const OperationalTaskSnapshot& tasks,
                       Eigen::VectorXd& qddot_nominal) = 0;
};
```

This keeps IDHQP representation-agnostic.
...
* [ ] Define the final `IDHierarchyPolicy`
* [ ] Define the final `IDSolution` contents
...
The final IDHQP should be built as a **block-based strict-priority inverse-dynamics HQP controller** centered on the following separation:
