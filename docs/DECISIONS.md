...
| 2026-06-15 | Split inverse-dynamics controller API from formulation implementation details. | Legacy TSID-style formulations mixed task/contact registry ownership, controller lifecycle, and HQP matrix assembly. | `wbc_core/controller/base` owns `IDBase` and `IDProblemRegistry`; `wbc_core/controller` owns `IDHQP`; `wbc_core/formulations` owns `IDProblem`, `IDSolution`, and implementation-level HQP blocks; legacy inverse-kinematics and acc-force inverse-dynamics formulations were removed. |
| 2026-06-15 | Build every inverse-dynamics problem through HQP helpers on `IDBase`. | Allowing separate formulation paths makes controller/formulation ownership ambiguous and lets solver style leak into runtime policy. | Derived ID controllers assemble `solvers::HQPData` via `IDBase::addConstraint` for hard feasibility terms and `IDBase::addTask` for weighted objectives/regularization; YAML may choose the inner QP backend, not a non-HQP problem path. |
